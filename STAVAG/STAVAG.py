...
def TVG_detection(
    ...
    Detect Temporally Variable Genes (TVGs) using regression on a 1D time coordinate.
     ...
     Args:
        ...
        sps (bool, optional): Whether to compute STAVAG priority scores by
            comparing with random feature importances. Defaults to ``False``.
        threshold (float, optional): Cutoff used when selecting TVGs. Increase
            this value to keep more genes. Defaults to ``0.05``.

    Returns:
        Dict[str, DataFrame]: Dictionary containing important genes over the time
        axis ``'T'``. The DataFrame for each key is already filtered with the threshold.
...
def gene_modules(adata: sc.AnnData, gene_list: Sequence[str]) -> Tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
    ...
    Returns:
        Tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
            ...
            corr: Gene to gene correlation matrix as a pandas DataFrame.
                Index and columns are gene names in ``gene_list``.

            df: Expression matrix of the selected genes as a pandas DataFrame.
